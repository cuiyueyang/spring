...
import com.example.demo.linkUtils.base.SystemPropertiesUtil;
...
public class NetUtil {
    ...
    protected static final String IPV4_LOCALHOST = "127.0.0.1";
    ...
    private static class LocalAddress {
        ...
        private void initLocalAddress() {
            ...
            if (localInetAddress == null || nic == null || localInetAddress.isLoopbackAddress()
                ...
                try {
                    localInetAddress = lookedUpAddr != null ? lookedUpAddr : InetAddress.getByName(IPV4_LOCALHOST);
        ...
        private static InetAddress findLocalAddressViaNetworkInterface() {
            ...
            try {
                for (Enumeration<NetworkInterface> allInterfaces = NetworkInterface
                    ...
                    if(isNicUpAndSupportsMulticast(nic)) {
                        // 检查是否符合prefer前缀
        ...
        // 检查网卡是否可用并支持广播
        private static boolean isNicUpAndSupportsMulticast(NetworkInterface nic) {
            boolean result = true;
            try {
