...
public class MoreValidate extends Validate {
    ...
    private static final String MSG_NON_NEGATIVE = ") must be >= 0";
    private static final String MSG_POSITIVE = ") must be > 0";
    ...
    public static int positive(@Nullable String role, int x) {
        if (x <= 0) {
            throw new IllegalArgumentException(role + " (" + x + MSG_POSITIVE);
        }
        return x;
    }
    ...
    public static double positive(@Nullable String role, double x) {
        if (!(x > 0)) { //NOSONAR not x < 0, to work with NaN.
            throw new IllegalArgumentException(role + " (" + x + MSG_POSITIVE);
        }
        return x;
    }
    ...
    public static int nonNegative(@Nullable String role, int x) {
        if (x < 0) {
            throw new IllegalArgumentException(role + " (" + x + MSG_NON_NEGATIVE);
        }
        return x;
    }
